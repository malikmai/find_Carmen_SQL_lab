...
WHERE countrycode = (SELECT code FROM country WHERE name = 'San Marino') 
  AND isofficial = 'T';
...
SELECT name 
FROM city 
WHERE countrycode = (SELECT code FROM country WHERE name = 'Vatican City') 
...
WHERE id = (SELECT capital 
            FROM country 
            WHERE code = (SELECT countrycode FROM city WHERE name = 'Vaticano'));
...
-- Carmen's final location is Encarnación, Paraguay